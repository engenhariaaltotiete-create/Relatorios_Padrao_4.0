...
import type { FinalWorkReport, ChecklistStatus, StoredFile } from '../types';
...
type Props={report:FinalWorkReport;onChange:(r:FinalWorkReport)=>void;onSaveBack:()=>void;onPdf:()=>void;onExport:()=>void;busy?:boolean};

export function FinalWorkEditor({report,onChange,onSaveBack,onPdf,onExport,busy}:Props){
  ...
  const patch=(partial:Partial<FinalWorkReport>)=>onChange({...report,...partial,updatedAt:new Date().toISOString()});
  ...
  return <main className="container editor">
    <div className="page-heading"><div><h2>Relatório Final de Obra em edição</h2><p>Salvamento local automático ativo.</p></div><button className="secondary" onClick={onSaveBack}>Salvar e voltar</button></div>
